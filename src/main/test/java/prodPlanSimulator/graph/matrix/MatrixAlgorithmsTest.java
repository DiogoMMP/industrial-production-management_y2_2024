package prodPlanSimulator.graph.matrix;
...
class MatrixAlgorithmsTest {
    ...
    public void testminDistGraph() {
        ...
        // Create a comparator and sum function for the edge weights
        Comparator<Integer> comparator = Integer::compare;
        BinaryOperator<Integer> sum = Integer::sum;

        // Compute the minimum distance graph
        MatrixGraph<String, Integer> minDistGraph = Algorithms.minDistGraph(completeMap, comparator, sum);

        // Verify the minimum distance graph
        assertNotNull(minDistGraph, "The minimum distance graph should not be null");
        assertEquals(completeMap.numVertices(), minDistGraph.numVertices(), "The number of vertices should be the same");

        // Check some specific distances
        assertEquals(Integer.valueOf(0), minDistGraph.edge("Porto", "Porto").getWeight(), "Distance from Porto to Porto should be 0");
        assertEquals(Integer.valueOf(75), minDistGraph.edge("Porto", "Aveiro").getWeight(), "Distance from Porto to Aveiro should be 75");
        assertEquals(Integer.valueOf(335), minDistGraph.edge("Porto", "Lisboa").getWeight(), "Distance from Porto to Lisboa should be 335");
        assertEquals(Integer.valueOf(365), minDistGraph.edge("Porto", "Castelo Branco").getWeight(), "Distance from Porto to Castelo Branco should be 365");
