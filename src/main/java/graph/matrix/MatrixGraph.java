...
public class MatrixGraph<V,E> extends CommonGraph<V,E> {
    ...
    public void insertEdge(V vOrig, V vDest, Object o) {
        if (vOrig == null || vDest == null) throw new RuntimeException("Vertices cannot be null!");
        if (edge(vOrig, vDest) != null)
            return;
        if (!validVertex(vOrig))
        ...
        edgeMatrix[vOrigKey][vDestKey] = new Edge<>(vOrig, vDest, (E) o);
        numEdges++;
        if (!isDirected) {
            edgeMatrix[vDestKey][vOrigKey] = new Edge<>(vDest, vOrig, (E) o);
            numEdges++;
        }
    }
