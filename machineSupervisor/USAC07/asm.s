...
get_n_elements:
    ...
    movl (%rcx), %ebx             # Load the value of head into %ebx

    subl $1, %esi                 # Decrement num by 1 (effective buffer size is num-1)

    cmpl %eax, %ebx               # Compare head (ebx) and tail (eax)
    je buffer_empty               # If head == tail, the buffer is empty

    movl %ebx, %ecx               # Copy head to ecx
...
not_wrapped:
    ...
    # Buffer has elements
    cmpl %eax, %ebx               # Check if head >= tail
    ...
    addl %ebx, %ecx               # ecx = num - tail + head
    jmp end

head_greater_or_equal_tail:
    subl %eax, %ebx               # ebx = head - tail
    movl %ebx, %ecx               # Store result in ecx
...
buffer_full:
    movl %esi, %eax               # Buffer is full, return num - 1
    jmp epilogue
